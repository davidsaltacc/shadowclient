...
public class FontTextureAtlas {
    ...
    public FontTextureAtlas(String font, int size) throws IOException {
        ...
        this.texture = new NativeImageBackedTexture(textureId.getPath(), ATLAS_SIZE, ATLAS_SIZE, true);
        MinecraftClient.getInstance().getTextureManager().registerTexture(textureId, texture);
        initializeTexture();

        initFreeType();
        fontBuffer = MiscUtils.resourceToByteBuffer(font);
    ...
    public Vector2i getSize() {
        return new Vector2i(ATLAS_SIZE, ATLAS_SIZE);
    }

    public record Glyph(int regionStartX, int regionStartY, int regionWidth, int regionHeight, int width, int height, int bearingX, int bearingY, int advance) {}
